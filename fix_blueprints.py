#!/usr/bin/env python
"""Fix missing Chemistry and Biology blueprints in Full NEET exam."""
import asyncio
import logging
...
async def fix_blueprints():
    ...
    async with AsyncSessionLocal() as db:
        ...
        if not full_exam:
            logger.error("Full NEET 2026 exam not found!")
            return False

        logger.info(f"Found Full NEET 2026 exam (id={full_exam.id})")
        ...
        added_count = 0
        for subject_name, topics in config.items():
            for topic_name, (expected_q, difficulty) in topics.items():
                # Find topic
                ...
                existing = (await db.execute(
                    select(ExamBlueprint).where(
                        ExamBlueprint.exam_id == full_exam.id,
                        ExamBlueprint.topic_id == topic.id
                    )
                )).scalar_one_or_none()

                if existing:
                    logger.info(f"✓ Blueprint already exists: {subject_name}/{topic_name}")
                    continue

                # Create blueprint
                bp = ExamBlueprint(
                    exam_id=full_exam.id,
                    topic_id=topic.id,
                ...
                db.add(bp)
                added_count += 1
                logger.info(f"+ Added blueprint: {subject_name}/{topic_name}")

        await db.commit()
        logger.info(f"\n✓ Fixed {added_count} missing blueprints")
        return True


if __name__ == "__main__":
    result = asyncio.run(fix_blueprints())
